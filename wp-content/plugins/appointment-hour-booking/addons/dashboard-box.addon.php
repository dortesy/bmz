...
if( !class_exists( 'CPAPPB_DashboardWidget' ) )
{
    class CPAPPB_DashboardWidget extends CPAPPB_BaseAddon
    {
		...
		public function pp_DashboardWidget($atts = array())
		{
            ...
            for($i=0; $i<count($selection) && $i<$maxitems; $i++)
            {
                ...
                if ($lastdate != $selection[$i][1])
                {
                    ...
                    // list header rows
                    $buffer .= '<th class="cpappbheadermth">'.esc_html($datelabel).'</th>';
                    $buffer .= '<td class="cpappbheadermtd" nowrap>'.esc_html($lastdate).'</td>';
                    $buffer .= '<tr>';
                    $colnum = 0;
                ...
                {    
                   switch ($fields[$j]) {
                    ...
                    $buffer .= '<td class="cpappbcol'.$colnum.'">'.esc_html(trim($value)).'</td>';
                }
                if ($showdelete)
                    $buffer .= '<td class="cpappbcol'.($colnum++).'">[<a href="javascript:cpappb_deleteitem('.intval($event->id).');">'.__('delete','appointment-hour-booking').'</a>]</td>';
                    $buffer .= '</tr>';               
            }   
		...
		private function _log($adarray = array())
		{
			...
			{
				$log .= sanitize_key($KEY).": ".esc_html($VAL)."\n";
			}
